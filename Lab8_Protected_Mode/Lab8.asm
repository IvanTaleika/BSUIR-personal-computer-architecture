...
;lidt/sidt - загрузка/сохранение содержимого IDTR в/из памяти
.386P
.MODEL  LARGE
;Структуры данных
...
WRITE_GDT:                                      ;Заполнить дескриптор GDT
    add ax,offset GDT
    adc dx,0
    mov GDT_GDT.BASE_L,ax
    mov GDT_GDT.BASE_M,dl
    mov GDT_GDT.BASE_H,dh
WRITE_CODE_RM:                                  ;Заполнить дескриптор сегмента кода реального режима
    ...
    shl ax,4
    shr dx,4
    mov GDT_CODE_RM.BASE_L,ax
    mov GDT_CODE_RM.BASE_M,dl
    mov GDT_CODE_RM.BASE_H,dh
WRITE_DATA:                                     ;Записать дескриптор сегмента данных
    mov ax,si
    mov dx,di
    mov GDT_DATA.BASE_L,ax
    mov GDT_DATA.BASE_M,dl
    mov GDT_DATA.BASE_H,dh
WRITE_STACK:                                    ;Записать дескриптор сегмента стека
    ...
    shl ax,4
    shr dx,4
    mov GDT_STACK.BASE_L,ax
    mov GDT_STACK.BASE_M,dl
    mov GDT_STACK.BASE_H,dh
WRITE_CODE_PM:                                  ;Записать дескриптор кода защищенного режима
    ...
    shl ax,4
    shr dx,4
    mov GDT_CODE_PM.BASE_L,ax
    mov GDT_CODE_PM.BASE_M,dl
    mov GDT_CODE_PM.BASE_H,dh        
    or  GDT_CODE_PM.ATTRIBS,40h
WRITE_IDT:                                      ;Записать дескриптор IDT
    mov ax,si
    mov dx,di
    add ax,OFFSET IDT
    adc dx,0
    mov GDT_IDT.BASE_L,ax
    mov GDT_IDT.BASE_M,dl
    mov GDT_IDT.BASE_H,dh        
    mov IDTR.IDT_L,ax
    mov IDTR.IDT_H,dx
...
;Исккусственно созданный дальний переход
;ip будет равен $+4 (текущий адрес + 4 = OVERLOAD_SEGMENT_REGISTERS)
;cs будет равен CODE_RM_DESC - селектору сегмента команд
...
GO_TO_CODE_PM:                                  ;Переход к сегменту кода защищенного режима
    db 66h                                      ;Интерпретировать код как 32-битные команды
    retf								;Значение метки уже в стеке
...
KEYBOARD_HANDLER proc near                      ;Обработчик прерывания клавиатуры
    push ax
    in al,60h		; AL содержит скан-код клавиатуры, не сохраняем его
    in al,61H
    mov ah,al
    or al,80h
    out 61H,al
...
DATA_BEGIN      = $
    ...
    MSG_HELLO_PM        db "welcome to protected mode",0
    MSG_EXIT            db "welcome back to real mode",13,10,"$"
    MSG_TIME            db "return RM in  XXXXXXX seconds",0
